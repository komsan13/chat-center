...
import { Calendar, Plus, Search, Edit2, Trash2, ArrowLeft, Filter, Banknote, Save, RefreshCw, Building2, Clock, CheckCircle, CreditCard, Globe, X, Sparkles, AlertTriangle } from 'lucide-react';
import Link from 'next/link';
import { useLanguage } from '@/contexts/LanguageContext';
import { useTheme } from '@/contexts/ThemeContext';
import { DeleteConfirmModal, ToastContainer, useToast, Modal, FormButton } from '@/components/ui';
...
export default function CashWithdrawalPage() {
  const [withdrawalData, setWithdrawalData] = useState<CashWithdrawal[]>([]);
  ...
  const [editingItem, setEditingItem] = useState<CashWithdrawal | null>(null);
  const [deletingItem, setDeletingItem] = useState<CashWithdrawal | null>(null);
  ...
  const [formErrors, setFormErrors] = useState<{ date?: string; bankId?: string; amount?: string }>({});
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/cash-withdrawal?${params.toString()}`);
  ...
  // Form validation
  const validateForm = () => {
    const errors: { date?: string; bankId?: string; amount?: string } = {};
    if (!formData.date) {
      errors.date = language === 'th' ? 'กรุณาเลือกวันที่' : 'Date is required';
    }
    if (!formData.bankId) {
      errors.bankId = language === 'th' ? 'กรุณาเลือกบัญชีธนาคาร' : 'Bank account is required';
    }
  ...
  // Stats
  const totalAmount = withdrawalData.reduce((sum, item) => sum + item.amount, 0);
  const totalFee = withdrawalData.reduce((sum, item) => sum + item.fee, 0);
  const pendingCount = withdrawalData.filter(item => item.status === 'pending').length;
  const completedCount = withdrawalData.filter(item => item.status === 'completed').length;
  ...
  const getSelectedBank = () => {
    return banksList.find(b => b.id === formData.bankId);
  };
  ...
  const openEditModal = (item: CashWithdrawal) => {
    setEditingItem(item);
    setFormData({
      date: item.date,
  ...
  const openDeleteModal = (item: CashWithdrawal) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingItem) {
        const response = await fetch(`/api/cash-withdrawal/${editingItem.id}`, {
          method: 'PUT',
        ...
        if (result.success) {
          ...
          showSuccess(
            ...
            language === 'th' ? 'ข้อมูลถอนเงินสดถูกแก้ไขเรียบร้อยแล้ว' : 'Cash withdrawal has been updated'
          );
        ...
        if (result.success) {
          ...
          showSuccess(
            ...
            language === 'th' ? 'ข้อมูลถอนเงินสดถูกเพิ่มเรียบร้อยแล้ว' : 'Cash withdrawal has been added'
          );
          setIsModalOpen(false);
        } else {
          showError(
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/cash-withdrawal/${deletingItem.id}`, {
        method: 'DELETE',
      });
      ...
      if (result.success) {
        ...
        showSuccess(
          ...
          language === 'th' ? 'ข้อมูลถอนเงินสดถูกลบเรียบร้อยแล้ว' : 'Cash withdrawal has been deleted'
        );
      } else {
        showError(
          language === 'th' ? 'เกิดข้อผิดพลาด' : 'Error',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div>
            <h1 style={{ fontSize: '24px', fontWeight: 700, color: colors.text, margin: 0 }}>
              {language === 'th' ? 'ถอนเงินสด' : 'Cash Withdrawal'}
            </h1>
            <p style={{ fontSize: '14px', color: colors.textMuted, margin: 0 }}>
              {language === 'th' ? 'จัดการรายการถอนเงินสด' : 'Manage cash withdrawal transactions'}
            </p>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          >
            ...
            <span>{language === 'th' ? 'แจ้งถอน' : 'New Withdrawal'}</span>
          </button>
        </div>
      </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div style={{
              ...
              <Banknote style={{ width: '20px', height: '20px', color: '#f59e0b' }} />
            </div>
            <div>
              <p style={{ fontSize: '13px', color: colors.textMuted, margin: 0 }}>{language === 'th' ? 'ยอดถอนรวม' : 'Total Withdrawal'}</p>
              <p style={{ fontSize: '20px', fontWeight: 700, color: '#f59e0b', margin: 0 }}>฿{formatCurrency(totalAmount)}</p>
            </div>
          </div>
        </div>
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              <p style={{ fontSize: '13px', color: colors.textMuted, margin: 0 }}>{language === 'th' ? 'ค่าธรรมเนียมรวม' : 'Total Fee'}</p>
              <p style={{ fontSize: '20px', fontWeight: 700, color: '#ef4444', margin: 0 }}>฿{formatCurrency(totalFee)}</p>
            </div>
          </div>
        </div>
        <div style={{
      ...
      <div style={{
        ...
        <div style={{ position: 'relative', flex: 1 }}>
          ...
          <input
            type="text"
            placeholder={language === 'th' ? 'ค้นหาธนาคาร, เลขบัญชี...' : 'Search bank, account...'}
            value={searchTerm}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          >
            ...
            <option value="pending">{language === 'th' ? 'แจ้งถอน' : 'Pending'}</option>
              ...
              <tr>
                <td colSpan={8} style={{ padding: '60px 16px', textAlign: 'center' }}>
                  <div style={{
                    ...
                    <Banknote style={{ width: '40px', height: '40px', color: '#f59e0b' }} />
                  </div>
                  <h3 style={{ fontSize: '18px', fontWeight: 600, color: colors.text, margin: '0 0 8px 0' }}>
                    {withdrawalData.length === 0
                      ? (language === 'th' ? 'ยังไม่มีรายการถอนเงิน' : 'No withdrawals yet')
                      : (language === 'th' ? 'ไม่พบข้อมูล' : 'No results found')
                    }
                  </h3>
                  <p style={{ fontSize: '14px', color: colors.textMuted, margin: 0 }}>
                    {withdrawalData.length === 0
                      ? (language === 'th' ? 'เริ่มต้นด้วยการแจ้งถอนรายการแรก' : 'Get started by creating your first withdrawal')
                  ...
                  {withdrawalData.length === 0 && (
                    ...
                    >
                      <Plus style={{ width: '18px', height: '18px' }} />
                      {language === 'th' ? 'แจ้งถอน' : 'New Withdrawal'}
                    </button>
                  )}
                </td>
              </tr>
              ...
              <tr key={item.id} style={{
                ...
                <td style={{ padding: '14px 16px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <span style={{ fontSize: '14px', fontWeight: 500, color: colors.text }}>{item.bankName}</span>
                  </div>
                </td>
                <td style={{ padding: '14px 16px' }}>
                  <div>
                    <p style={{ fontSize: '14px', fontWeight: 500, color: colors.text, margin: 0, fontFamily: 'monospace' }}>{item.accountNumber}</p>
                    <p style={{ fontSize: '12px', color: colors.textMuted, margin: '2px 0 0 0' }}>{item.accountName}</p>
                  </div>
                </td>
                <td style={{ padding: '14px 16px' }}>
        ...
        <div style={{
          ...
          <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                <div style={{
                  ...
                  {editingItem ? (
                    ...
                    <Banknote style={{ width: '24px', height: '24px', color: '#fff' }} />
                  )}
                </div>
                <div>
                  <h2 style={{ fontSize: '20px', fontWeight: 700, color: colors.text, margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
                    {editingItem
                      ? (language === 'th' ? 'แก้ไขรายการ' : 'Edit Withdrawal')
                      : (language === 'th' ? 'แจ้งถอนเงินสด' : 'New Cash Withdrawal')
                    }
                    {!editingItem && <Sparkles style={{ width: '18px', height: '18px', color: '#f59e0b' }} />}
                  </h2>
                  <p style={{ fontSize: '14px', color: colors.textMuted, margin: '4px 0 0 0' }}>
                    {editingItem
                      ? (language === 'th' ? 'แก้ไขข้อมูลการถอนเงิน' : 'Update withdrawal details')
                      : (language === 'th' ? 'กรอกข้อมูลการถอนเงินสด' : 'Fill in withdrawal details')
                    }
                  </p>
                </div>
              </div>
            ...
            <div style={{ padding: '24px 28px', overflowY: 'auto', flex: 1 }}>
              ...
              <div style={{ marginBottom: '20px' }}>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, color: colors.text, marginBottom: '8px' }}>
                  {language === 'th' ? 'บัญชีธนาคาร' : 'Bank Account'} <span style={{ color: '#ef4444' }}>*</span>
                </label>
                <div style={{
                  padding: '12px',
                  borderRadius: '12px',
                  border: `2px solid ${formErrors.bankId ? '#ef4444' : colors.border}`,
                  background: colors.inputBg,
                  display: 'flex',
                  flexDirection: 'column',
                  gap: '8px',
                  ...
                  {banksList.length === 0 ? (
                    <p style={{ fontSize: '13px', color: colors.textMuted, margin: 0 }}>
                      {language === 'th' ? 'ไม่มีบัญชีธนาคารในระบบ' : 'No bank accounts available'}
                    </p>
                  ) : (
                    banksList.map((bank) => {
                      const isSelected = formData.bankId === bank.id;
                      return (
                        <button
                          key={bank.id}
                          type="button"
                          onClick={() => {
                            ...
                            if (formErrors.bankId) setFormErrors({ ...formErrors, bankId: undefined });
                          }}
                          style={{
                            display: 'flex',
                            alignItems: 'center',
                        ...
                        >
                          <div style={{
                            ...
                            height: '36px',
                            borderRadius: '8px',
                            background: isSelected ? 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)' : colors.cardBg,
                            display: 'flex',
                            alignItems: 'center',
                            justifyContent: 'center',
                            flexShrink: 0,
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <p style={{ fontSize: '14px', fontWeight: 600, color: isSelected ? '#f59e0b' : colors.text, margin: 0 }}>
                              {bank.bankName}
                            </p>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
                            <span style={{
                              ...
                              fontSize: '11px',
                              fontWeight: 600,
                              background: bank.type === 'deposit' ? 'rgba(34, 197, 94, 0.15)' 
                                : bank.type === 'withdrawal' ? 'rgba(239, 68, 68, 0.15)'
                                : bank.type === 'payment' ? 'rgba(59, 130, 246, 0.15)'
                                : 'rgba(245, 158, 11, 0.15)',
                              color: bank.type === 'deposit' ? '#22c55e' 
                                : bank.type === 'withdrawal' ? '#ef4444'
                                : bank.type === 'payment' ? '#3b82f6'
                                : '#f59e0b',
                            }}>
                              {bank.type === 'deposit' ? (language === 'th' ? 'ฝาก' : 'Deposit') 
                                : bank.type === 'withdrawal' ? (language === 'th' ? 'ถอน' : 'Withdraw')
                                : bank.type === 'payment' ? (language === 'th' ? 'ชำระเงิน' : 'Payment')
                                : (language === 'th' ? 'พักเงิน' : 'Hold')}
                            </span>
                            {isSelected && (
      ...
      <DeleteConfirmModal
        ...
        subtitle={language === 'th' ? 'คุณกำลังจะลบรายการถอนเงินนี้' : 'You are about to delete this withdrawal'}
        itemName={deletingItem ? `${formatDate(deletingItem.date)} - ${deletingItem.bankName}` : ''}
        itemDetails={deletingItem ? [
          { label: language === 'th' ? 'จำนวน' : 'Amount', value: `฿${formatCurrency(deletingItem.amount)}` }
        ] : []}
